 ...
 */
const patchValueTypeDefinitionMap = (values: ValueDefinitionMap) => {
 ...
 *
 * class MyController extends typedController(Controller, { values, targets, outlets }) {
 *  // Look Ma, no "declare ..."
...
>(Base: Base, statics: Statics<Values, Targets, Outlets> = {}) => {
  ...
  const derived = class extends Base {
    static customTargets = (targets ?? []) as Targets;
    static customValues = (values ?? {}) as Values;
    static customOutlets = (outlets ?? []) as Outlets;
